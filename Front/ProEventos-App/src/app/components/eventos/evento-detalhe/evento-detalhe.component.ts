...
export class EventoDetalheComponent implements OnInit {
      ...
      if(this.eventoId !== null && this.eventoId !== 0){
        ...
        this.eventoService.getEventoById(this.eventoId).subscribe({
          next:(eventoRetorno: Evento) => {
            ...
            this.evento.lotes.forEach(lote => // esta chamada substitui a chamada do carregarLotes()
  ...
  public salvarEvento(): void{
    if(this.form.valid){
      this.spinner.show();
      this.evento = this.statusSalvar === 'post' ? {... this.form.value} :
        ...
        this.eventoService[this.statusSalvar](this.evento).subscribe(
          (eventoRetorno: Evento) => {
            ...
            this.router.navigate([`eventos/detalhes/${eventoRetorno.eventoId}`])
          },
          (error: any) => {
            console.error(error),
  ...
  criarLotes(lote: Lote): FormGroup {
    return this.fb.group({
      ...
      quantidade: [lote.quantidade, [Validators.required, Validators.min(99), Validators.max(500)]],
  ...
  confirmeDelete(): void{
    ...
    this.loteService.deleteLote(this.eventoId, this.loteAtual.id).subscribe({
      next: (result: string) => {
        this.toastr.success('Evento deletado com sucesso!', 'Evento deletado!');
